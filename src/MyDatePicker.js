...
let todayTimestampa = new Date("2021-5-21");
let refDate = Date.now() - (Date.now() % oneDay) + (new Date().getTimezoneOffset() * 1000 * 60);
...
export default class MyDatePicker extends Component {
    ...
    constructor() {
        ...
        this.state = {
            year,
            month,
            selectedDay: todayTimestampa,
            monthDetails: this.getMonthDetails(year, month),
            timeElapsed: todayTimestamp - todayTimestampa
        }
    }
    ...
    updateDateFromInput =()=> {
        ...
        if(dateData !== null) { 
            console.log(this.setDate(dateData));
            this.setState({ 
                year: dateData.year, 
    ...
    render() {
        return (
            <div className='MyDatePicker'>
                ...
                <div>
                    <div className='mdp-input'>
                        <input type='text' value="Time elapsed" />
                    </div>
                    <div className='mdp-input'  onClick={()=> this.showDatePicker(true)}>
                        <input type='text' value={this.state.timeElapsed } />
                    </div>
                </div>
